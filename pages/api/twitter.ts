...
export default async function handler(req: VercelRequest, res: VercelResponse) {
	...
	if (req.method === "POST") {
		...
		const query_params: QueryParams = {
			...
			"tweet.fields":
				"created_at,public_metrics,possibly_sensitive,in_reply_to_user_id,geo,entities",
		...
		const twitter_api_response: Response = await fetch(
			api_req_link,
			twitter_api_request
		);

		const api_resp = await twitter_api_response.json();
		const pfp_link = api_resp["includes"]["users"][0]["profile_image_url"];

		res.status(200).json(api_resp);
		console.debug(`${pfp_link}`);
	} else {
